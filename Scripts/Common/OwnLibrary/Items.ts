...
function getCargoInfoItem(id: number, propertiesFreight: JQuery, linerTerm: JQuery, description: string, packing: JQuery, length: number, height: number, width: number, measureDimentions: JQuery, imoClass: JQuery, volume: number, measureVolume: JQuery, weight: number, measureWeight: JQuery, quantity: number, typeOfCargo: string): any {
    ...
    let lengthX = length * quantity;
    let heightX = height * quantity;
    let widthX = width * quantity;
    ...
    let dimentionsHtml = typeOfCargo != 'FCL' ? length + ' x ' + height + ' x ' + width + ' ' + measureDimentions.find(':selected').text() + ' -> ' + lengthX + ' x ' + heightX + ' x ' + widthX + ' ' + measureDimentions.find(':selected').text() : 'Sin dimensiones establecidas';
...
function createTrackingOrder(index: number, number: string, shipper: JQuery, isProspect: boolean, incoterm: JQuery, warehouse: string, dateWarehouse: JQuery, trackingId: number, id?: number): any {
    ...
    let tNumber = number == '' ? 'Sin número de pedido' : 'Pedido #' + number;
    let tShipper = (shipper.val() == '' || shipper.val() == null) ? 'Sin shipper' : shipper.find(':selected').text();
    let tWarehouse = warehouse == '' ? 'El Warehouse aun es desconocido.' : 'El pedido ingresa al Warehouse <strong>' + warehouse + ' </strong>.';
    let tDateWarehouse = dateWarehouse.val() == '' ? 'La fecha de Ingreso es desconocida.' : 'Ingresará el <strong>' + new Date(dateWarehouse.val()).toLocaleString() + '</strong>.';
